...
tiny_yolo_graph_file= "graph" #Done: Assign this to the name of the pretrained yolo graph file. 
...
def save_boxes(boxes,image):
    images=[]
    for box in boxes:
        ##Crops an image to the box
        box_image=image[box[0]-0.5*box[3]:box[0]+0.5*box[3] , box[1]-0.5*box[2]:box[1]+0.5*box[2]]
        global i
        filename="task7_images\Image {}.jpg".format(i)
...
def main():
    ...
    devices = mvnc.enumerate_devices() #Done use the mvnc API to querydevices. 
    if len(devices) == 0:
        print('No devices found')
        return 1
    device = mvnc.enumerate_devices()[0] #Done: use the mvnc API to assign the first device in devices to the device variable. 
    device.open()
    ...
    #Done: Instantiate fifo_in and fifo_out using the graph file above. 
    ...
    while True:
        ...
        graph.queue_inference_with_fifo_elem(fifo_in,fifo_out, input_image, user_obj)

        #Done: Use the queue_inference_with_fifo_elem to load the image and get the result from the NCS. This should be one line of code.
        ...
        ##Saving the boxes to file
        save_boxes(filtered_objs,display_image)
